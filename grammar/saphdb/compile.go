...
func (grammarSQL Hdb) CompileSelect(query *dbal.Query) string {
	bindingOffset := 0
	return grammarSQL.CompileSelectOffset(query, &bindingOffset)
}

// CompileSelectOffset Compile a select query into SQL.
func (grammarSQL Hdb) CompileSelectOffset(query *dbal.Query, offset *int) string {
...
func (grammarSQL Hdb) CompileColumns(query *dbal.Query, columns []interface{}, bindingOffset *int) string {
...
func (grammarSQL Hdb) CompileWheres(query *dbal.Query, wheres []dbal.Where, bindingOffset *int) string {
...
func (grammarSQL Hdb) WhereDate(query *dbal.Query, where dbal.Where, bindingOffset *int) string {
	value := ""
...
func (grammarSQL Hdb) WhereTime(query *dbal.Query, where dbal.Where, bindingOffset *int) string {
	value := ""
	if !dbal.IsExpression(where.Value) {
		*bindingOffset = *bindingOffset + where.Offset
		value = grammarSQL.Parameter(where.Value, *bindingOffset)
...
func (grammarSQL Hdb) WhereDay(query *dbal.Query, where dbal.Where, bindingOffset *int) string {
	return grammarSQL.WhereDateBased("day", query, where, bindingOffset)
}

// WhereMonth Compile a "where month" clause.
func (grammarSQL Hdb) WhereMonth(query *dbal.Query, where dbal.Where, bindingOffset *int) string {
	return grammarSQL.WhereDateBased("month", query, where, bindingOffset)
}

// WhereYear Compile a "where year" clause.
func (grammarSQL Hdb) WhereYear(query *dbal.Query, where dbal.Where, bindingOffset *int) string {
	return grammarSQL.WhereDateBased("year", query, where, bindingOffset)
}

// WhereDateBased  Compile a date based where clause.
func (grammarSQL Hdb) WhereDateBased(typ string, query *dbal.Query, where dbal.Where, bindingOffset *int) string {
	value := ""
	if !dbal.IsExpression(where.Value) {
		*bindingOffset = *bindingOffset + where.Offset
		value = grammarSQL.Parameter(where.Value, *bindingOffset)
...
func (grammarSQL Hdb) CompileLock(query *dbal.Query, lock interface{}) string {
	lockType, ok := lock.(string)
	if ok == false {
		return ""
	} else if lockType == "share" {
		return "for share lock"
...
func (grammarSQL Hdb) CompileExists(query *dbal.Query) string {
	sql := grammarSQL.CompileSelect(query)
	return fmt.Sprintf("select count( * ) as %s FROM (%s LIMIT 1)", grammarSQL.Wrap("exists"), sql)
}
